...
const AuthNavigator = () => {
  ...
  const authContext = useMemo(
    () => ({
      signIn: async data =>
        dispatch({type: AUTH_SIGN_IN, token: 'dummy-auth-token'}),
      signUp: async data =>
        dispatch({type: AUTH_SIGN_OUT, token: 'dummy-auth-token'}),
  ...
  const appStackScreen = () => {
    return <Stack.Screen name="Home" component={routes.HOME.component} />;
  };
